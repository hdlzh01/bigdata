...
def RecommendMovies(model,movieTitle,inputUserId):
    RecommendMovie = model.recommendProducts(inputUserId, int(input_[1]))
    print("对用户ID为"+str(inputUserId)+"的用户推荐下列"+input_[1]+"部电影：")
...
def RecommendUsers(model,movieTitle,inputMovieId):
    RecommendUser = model.recommendUsers(inputMovieId, int(input_[1]))
    print("对电影ID为"+str(inputMovieId)+"的电影推荐给下列"+input_[1]+"个用户：")
...
def Recommend(model):
    """根据参数进行推荐"""
    if input_[0][0] == "U":
        RecommendMovies(model, movieTitle, int(input_[0][1:]))
    if input_[0][0] == "M":
        RecommendUsers(model, movieTitle, int(input_[0][1:]))
